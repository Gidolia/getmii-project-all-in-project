...
if($countddd->num_rows > 0){
    ...
    $pro="SELECT u_id,shop_name,shop_city FROM `user` WHERE `u_id`='$_POST[u_id]'";
    $prod=$con->query($pro);
    $s_fet=$prod->fetch_assoc();
    
    $df="INSERT INTO `coupan_shop` (`cs_id`, `coun_id`, `shop_id`, `c_date`, `c_time`, `e_date`, `status`, `discount`, `data`, `city`) VALUES (NULL, '$_POST[coun_id]', '$_POST[u_id]', '$date', '$time', '$e_date', '1', '$_POST[amount]', '$_POST[data]', '$s_fet[shop_city]');";
