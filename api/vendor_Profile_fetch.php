...
$dc=$con->query($ro);
     ...
     {      $fetch=$dc->fetch_assoc();
         ...
         $response[]=array("u_id"=>"$fetch[u_id]", "name"=>"$fetch[name]","owner_name"=>"$fetch[owner_name]","u_wallet"=>"$fetch[u_wallet]","shop_name"=>"$fetch[shop_name]","shop_mobile"=>"$fetch[shop_mobile]","bank_ifsc"=>"$fetch[ifsc]",
         "bank_acc_no"=>"$fetch[bank_acc_no]","bank_name"=>"$fetch[bank_name]", "shop_address"=>"$fetch[shop_addreass]","shop_city"=>"$fetch[shop_city]",
