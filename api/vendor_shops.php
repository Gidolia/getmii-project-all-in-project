...
$de="SELECT u_id,shop_name,shop_city,owner_name,shop_mobile,shop_addreass,	csc_id, shop_logo,trusted_status,paid_status FROM `user` WHERE `shop_name`!='' AND `shop_city` LIKE '%$_GET[city]%' AND `csc_id` = '$_GET[c_id]' AND `approval_status`!='p' AND `shop_status`='1'";
...
$dc=$con->query($de);
     if($dc->num_rows>0)
     {
         while($fetch=$dc->fetch_assoc()){
             ...
             $liap="SELECT * FROM `shop_like_comment` WHERE `shop_id`='$fetch[u_id]' AND `like`='1' AND `u_id`='$_GET[u_id]'";
             $acpp=$con->query($liap);
             if($acpp->num_rows>0)
             {$followed=1;}else{
                 $followed=0;
       ...
       $data[]=array("message"=>"0", "c_id"=>"$_GET[c_id]");
